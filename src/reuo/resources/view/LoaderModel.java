...
/**
 * Provides a synchrnous {@link ListModel} interface to a {@link Loader}. When an element
 * is requested via {@link #getElementAt(int)} it will be loaded from the underlying loader,
 * which may have the resource cached. This is only recommened for loaders that offer very
 * quick access, because this will block the AWT event thread.
 * @author Kristopher Ives
 * @see AsyncLoaderModel
 */
public class LoaderModel implements ListModel{
	...
	/** A mapping between valid resource ids and the indices exposed to the ListModel */
	...
	/**
	 * Initializes a LoaderModel with the specified loader using <code>indices</code>
	 * as a source for valid entry identifiers.
	 * @param loader the underlying loader
	 * @param indices the source for valid entry identifiers
	 */
	 ...
	 * @param index the ListModel index
