 ...
 * <code>Buffer</code> the pixels are stored in may be direct or non-direct
 * and is not garuanteed to be a specific type of <code>Buffer</code>.
 * 
...
public class Palette implements Loadable{
	...
	public void load(ByteBuffer in) throws IllegalArgumentException{
		if(formatter != null){
			data = formatter.format(in.duplicate().order(ByteOrder.LITTLE_ENDIAN), size, 1, null);
		}else{
			// TODO might be a bug PALETTE
			data = in.duplicate().order(ByteOrder.LITTLE_ENDIAN).limit(in.position() + size * 2);
		}
